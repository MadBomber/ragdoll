  ...
  module Core
    ...
    class ModelResolver
      ...
      def resolve_for_task(task_type, content_type = :text)
        ...
        raise ConfigurationError, "No model configured for task '#{task_type}'" if model_string.nil?
      ...
      def resolve_embedding(content_type = :text)
        ...
        raise ConfigurationError, "No embedding model configured for content type '#{content_type}'" if model_string.nil?

        model = Model.new(model_string)
